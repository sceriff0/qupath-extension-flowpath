...
import qupath.ext.quflow.engine.LivePreviewService;
import qupath.ext.quflow.io.QuFlowSerializer;
import qupath.ext.quflow.io.PhenotypeCsvExporter;
import qupath.ext.quflow.model.*;
 ...
 */
public class QuFlowPane extends BorderPane {
    ...
    public QuFlowPane(QuPathGUI qupath) {
        this.qupath = qupath;
        ...
        treeView.setCellFactory(tv -> new QuFlowCell());
    ...
    private void initializeFromImage() {
        ...
        if (detections.isEmpty()) {
            Dialogs.showWarningNotification("QuFlow", "No detections found. Import GeoJSON cells first.");
            return;
        }
    ...
    private TreeItem<Object> buildTreeItem(GateNode gate) {
        ...
        // Positive branch
        TreeItem<Object> posItem = new TreeItem<>(new QuFlowCell.BranchItem(gate, true));
        ...
        // Negative branch
        TreeItem<Object> negItem = new TreeItem<>(new QuFlowCell.BranchItem(gate, false));
    ...
    private void addRootGate() {
        if (markerNames == null || markerNames.isEmpty()) {
            Dialogs.showWarningNotification("QuFlow", "No markers available. Load an image with detections first.");
            return;
        }
    ...
    private void onTreeSelectionChanged(TreeItem<Object> selected) {
        ...
        } else if (item instanceof QuFlowCell.BranchItem branch) {
    ...
    private GateNode getSelectedGateNode() {
        ...
        if (item instanceof QuFlowCell.BranchItem branch) return branch.parentGate;
        return null;
    }
    ...
    private void saveTree() {
        File file = Dialogs.promptToSaveFile("Save QuFlow", null, "quflow.json", "JSON", ".json");
        if (file == null) return;
        try {
            QuFlowSerializer.save(gateTree, file);
            Dialogs.showInfoNotification("QuFlow", "Saved to " + file.getName());
        } catch (Exception ex) {
    ...
    private void loadTree() {
        File file = Dialogs.promptForFile("Load QuFlow", null, "JSON", ".json");
        if (file == null) return;
        try {
            gateTree = QuFlowSerializer.load(file);
            ...
            Dialogs.showInfoNotification("QuFlow", "Loaded from " + file.getName());
        } catch (Exception ex) {
    ...
    private void exportCsv() {
        if (cellIndex == null || markerStats == null || gateTree.getRoots().isEmpty()) {
            Dialogs.showWarningNotification("QuFlow", "No gates defined or no cells loaded.");
            return;
        }
        ...
        try {
            ...
            Dialogs.showInfoNotification("QuFlow", "Exported " + file.getName());
        } catch (Exception ex) {
