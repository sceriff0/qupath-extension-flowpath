...
class NewGateTypesTest {
    ...
    @Test
    void booleanAndRequiresAllPositive() {
        // 4 cells: CD45 values {1, 6, 1, 6}, CD3 values {1, 1, 6, 6}
        // Operand1: CD45 threshold 5.0, Operand2: CD3 threshold 5.0
        // AND: only cell 3 (CD45=6 >= 5, CD3=6 >= 5) should match
        List<String> markers = List.of("CD45", "CD3");
        double[][] values = {
            {1, 6, 1, 6},  // CD45
            {1, 1, 6, 6}   // CD3
        ...
        GateNode op1 = new GateNode("CD45", 5.0);
        op1.setThresholdIsZScore(false);
        GateNode op2 = new GateNode("CD3", 5.0);
        op2.setThresholdIsZScore(false);

        BooleanGate gate = new BooleanGate(BooleanGate.Op.AND, "CD45+CD3+");
        gate.setOperands(List.of(op1, op2));
        ...
        // Cell 0: CD45=1 < 5, CD3=1 < 5 -> no match
        assertEquals(gate.getNoMatchBranch().getName(), phenotypes[0], "Cell 0 should be no match");
        // Cell 1: CD45=6 >= 5, CD3=1 < 5 -> no match
        assertEquals(gate.getNoMatchBranch().getName(), phenotypes[1], "Cell 1 should be no match");
        // Cell 2: CD45=1 < 5, CD3=6 >= 5 -> no match
        assertEquals(gate.getNoMatchBranch().getName(), phenotypes[2], "Cell 2 should be no match");
        // Cell 3: CD45=6 >= 5, CD3=6 >= 5 -> match
        assertEquals(gate.getMatchBranch().getName(), phenotypes[3], "Cell 3 should be match");
    }

    @Test
    void booleanOrRequiresAnyPositive() {
        // Same data as AND test. OR: cells 1, 2, 3 should match (any operand passes)
        List<String> markers = List.of("CD45", "CD3");
        double[][] values = {
            {1, 6, 1, 6},  // CD45
            {1, 1, 6, 6}   // CD3
        ...
        BooleanGate gate = new BooleanGate(BooleanGate.Op.OR, "CD45orCD3");
        gate.setOperands(List.of(op1, op2));
        ...
        // Cell 0: neither passes -> no match
        assertEquals(gate.getNoMatchBranch().getName(), phenotypes[0], "Cell 0 should be no match");
        // Cell 1: CD45 passes -> match
        assertEquals(gate.getMatchBranch().getName(), phenotypes[1], "Cell 1 should be match");
        // Cell 2: CD3 passes -> match
        assertEquals(gate.getMatchBranch().getName(), phenotypes[2], "Cell 2 should be match");
        // Cell 3: both pass -> match
        assertEquals(gate.getMatchBranch().getName(), phenotypes[3], "Cell 3 should be match");
    }

    @Test
    void booleanNotInvertsResult() {
        // 4 cells: CD45 values {1, 3, 7, 9}. Operand: CD45 threshold 5.0.
        // NOT: cells that are positive (>= 5) become no match; negative (< 5) become match.
        ...
        GateNode operand = new GateNode("CD45", 5.0);
        operand.setThresholdIsZScore(false);

        BooleanGate gate = new BooleanGate(BooleanGate.Op.NOT, "NotCD45");
        gate.setOperands(List.of(operand));
        ...
        // Cells 0,1: CD45 < 5 -> operand negative -> NOT inverts -> match
        assertEquals(gate.getMatchBranch().getName(), phenotypes[0], "Cell 0 (CD45=1 < 5) should be match (inverted)");
        assertEquals(gate.getMatchBranch().getName(), phenotypes[1], "Cell 1 (CD45=3 < 5) should be match (inverted)");
        // Cells 2,3: CD45 >= 5 -> operand positive -> NOT inverts -> no match
        assertEquals(gate.getNoMatchBranch().getName(), phenotypes[2], "Cell 2 (CD45=7 >= 5) should be no match (inverted)");
        assertEquals(gate.getNoMatchBranch().getName(), phenotypes[3], "Cell 3 (CD45=9 >= 5) should be no match (inverted)");
    }

    @Test
    void booleanDeepCopyIndependence() {
        GateNode op1 = new GateNode("CD45", 3.0);
        GateNode op2 = new GateNode("CD3", 4.0);

        BooleanGate original = new BooleanGate(BooleanGate.Op.AND, "TestBool");
        original.setOperands(new ArrayList<>(List.of(op1, op2)));
        original.setClipPercentileLow(5.0);

        BooleanGate copy = (BooleanGate) original.deepCopy();

        // Verify copy matches
        assertEquals(BooleanGate.Op.AND, copy.getOperation());
        assertEquals(2, copy.getOperands().size());
        assertEquals(5.0, copy.getClipPercentileLow());

        // Modify original
        original.setOperation(BooleanGate.Op.OR);
        original.getOperands().clear();
        original.setClipPercentileLow(10.0);

        // Copy should be unchanged
        assertEquals(BooleanGate.Op.AND, copy.getOperation(), "Copy operation should not change");
        assertEquals(2, copy.getOperands().size(), "Copy operands should not change");
        assertEquals(5.0, copy.getClipPercentileLow(), "Copy clipPercentileLow should not change");
    }
    ...
    @Test
    void booleanGateSerializationRoundTrip() throws IOException {
        GateNode op1 = new GateNode("CD45", 1.5);
        op1.setThresholdIsZScore(false);
        GateNode op2 = new GateNode("CD3", 2.5);
        op2.setThresholdIsZScore(true);

        BooleanGate gate = new BooleanGate(BooleanGate.Op.OR, "CD45orCD3");
        gate.setOperands(List.of(op1, op2));
        gate.setClipPercentileLow(3.0);
        gate.setClipPercentileHigh(97.0);

        GateTree tree = new GateTree();
        // default QualityFilter (passes all)
        tree.addRoot(gate);

        File file = tempDir.resolve("boolean.json").toFile();
        FlowPathSerializer.save(tree, file);
        GateTree loaded = FlowPathSerializer.load(file);

        assertEquals(1, loaded.getRoots().size());
        assertInstanceOf(BooleanGate.class, loaded.getRoots().get(0));

        BooleanGate loadedGate = (BooleanGate) loaded.getRoots().get(0);
        assertEquals(BooleanGate.Op.OR, loadedGate.getOperation());
        assertEquals(3.0, loadedGate.getClipPercentileLow());
        assertEquals(97.0, loadedGate.getClipPercentileHigh());

        // Verify operands
        assertEquals(2, loadedGate.getOperands().size());
        assertEquals("CD45", loadedGate.getOperands().get(0).getChannel());
        assertEquals(1.5, loadedGate.getOperands().get(0).getThreshold());
        assertFalse(loadedGate.getOperands().get(0).isThresholdIsZScore());
        assertEquals("CD3", loadedGate.getOperands().get(1).getChannel());
        assertEquals(2.5, loadedGate.getOperands().get(1).getThreshold());
        assertTrue(loadedGate.getOperands().get(1).isThresholdIsZScore());

        // Verify branches preserved
        assertEquals(2, loadedGate.getBranches().size());
        assertEquals(gate.getMatchBranch().getName(), loadedGate.getMatchBranch().getName());
        assertEquals(gate.getNoMatchBranch().getName(), loadedGate.getNoMatchBranch().getName());
    }
}
